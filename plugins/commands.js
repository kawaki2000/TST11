...
Module({
    ...
    for (const n of types) {
        for (const x of cmd_obj[n]) {
            ...
            final += `${final.includes(newn)?'':'\n\n╭❏═══〘 *_'+newn+"_* 〙❏═══✰❏"}\n┆ ❏ _${i}. ${handlerPrefix}${x.trim()}_${cmd_obj[n]?.indexOf(x)===(cmd_obj[n]?.length-1) ?`\n┃${star}╰─────────────────❏\n╰❏═════════════════✰❏`:''}`
        }
    }
    ...
    const botImageLink = BOT_INFO.split(";")[3] || `https://files.catbox.moe/96cfkn.jpg`
...
│❏ _*Mode*_ : ${MODE}
│ > *DEVELOPED By 𝙺𝙰𝚆𝙰𝙺𝙸*
│❏ _*Available RAM*_ : ${used} of ${total}
│ ❏ _*User*_ : ${message.senderName.replace(/[\r\n]+/gm, "")}
│ >  *𝒎𝒂𝒌𝒊𝒎𝒂-𝒎𝒅🌹*
│❏   𝑻𝑺𝑻-𝑩𝑶𝑻 
│❏/></🅣/></🅢/></🅣
└─────────────┈⳹
