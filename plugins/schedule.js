...
Module({
    ...
    if (!match[1]) {
        return await m.sendReply('_Please provide JID and time_\n\n*Example:*\n• schedule 22782639423@s.whatsapp.net 2h\n• schedule 22782639423@lid 2h\n• schedule 30m 22782639423@s.whatsapp.net');
    }
    ...
    try {
        ...
        await m.sendReply(`✅ *Message scheduled successfully!*\n\n📅 *Time:* ${formattedTime}\n⏰ *From now:* ${timeFromNow}\n📱 *Target:* ${jid}`);
    } catch (error) {
...
Module({
    ...
    try {
        ...
        if (success) {
            await m.sendReply(`✅ *Scheduled message ${messageId} cancelled successfully*`);
        } else {
